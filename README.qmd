...
Pour i allant de 1 à 12, on calcule le pourcentage d'erreurs commises par les deux arbres dont la profondeur est fixée à i (le pourcentage d'erreurs est calculé grâce au score donné par le module **score** de la fonction **DecisionTreeClassifier**).
...
plt.figure()
plt.plot(np.arange(dmax), 1-scores_entropy, label="entropy")
plt.plot(np.arange(dmax), 1-scores_gini, label="gini")
...
Affichons l'arbre de décision à 12 nœuds estimé avec *X_train* et *y_train* de notre ensemble d'apprentissage *data4*.
...
Pour i allant de 1 à 12, on calcule le pourcentage d'erreurs de l'estimation de *y_test* par les arbres au critère de gini et au critère d'entropie de profondeurs i (entraînés avec *X_train* et *y_train*).
