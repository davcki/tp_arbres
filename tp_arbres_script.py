...
for i in range(dmax):
    ...
    plt.subplot(3, 4, i + 1)
    frontiere(lambda x: dt_gini.predict(x.reshape((1, -1))), X_test, y_test, step=50, samples=False)
plt.draw()
...
plt.figure()
frontiere(lambda x: dt_entropy.predict(x.reshape((1, -1))), X_test, y_test, step=100)
plt.title("Best frontier with entropy criterion")
plt.draw()
print("Best scores with entropy criterion: ", dt_entropy.score(X_test, y_test))
...
data_test = rand_checkers(50, 50, 50, 50, sigma)
